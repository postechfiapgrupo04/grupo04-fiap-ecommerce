...
@Log
@Component
public class AuthAdminFilter implements GatewayFilter {

    private final ValidateTokenService validateTokenService;

    public AuthAdminFilter(ValidateTokenService validateTokenService) {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if(Objects.nonNull(userDTOMono)) {
            if (userDTOMono.authorities().stream().anyMatch(any -> any.authority().equals("ROLE_ADMIN"))) {
